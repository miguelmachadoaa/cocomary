...
class Application_Model_DbTable_Pedidos extends Zend_Db_Table_Abstract {
    ...
    public function getPedido($id) {
        $select = $this->select()
                        ->from(array('e'  => 'hk_pedidos'),array('id','id_cliente' ,'total','fecha'=>'DATE_FORMAT(fecha, "%d/%m/%Y")','status', 'descripcion')  )
                        ->where('id = ?', $id);
    ...
    public function updatePedidoCosto($id) {
       
       $select = $this->select()
            ->from(array('e'  => 'hk_detalle_pedido'),array('total_detalle_pedido'=>'sum(costo)')  )
            ->group('id_pedido')
            ->where('id_pedido = "'.$id.'"');
        ...
        $data = array('costo' => $row->total_detalle_pedido );
    ...
    public function getPedidos($options = array()) {
        ...
        $select = $this->select()
             ->from(array('e'  => 'hk_pedidos'),array('id','id_cliente','id_user' ,'total','fecha'=>'DATE_FORMAT(fecha, "%d/%m/%Y")','status','descripcion_pedido'=>'descripcion')  )
             ->join(array('s' => 'hk_estatus_pedido'),'e.status = s.id', array('descripcion', 'clase', 'id_estatus_pedido'=>'id'))
            ->join(array('c' => 'hk_clientes'),'e.id_cliente = c.id', array('nombre', 'tlf', 'direccion'))
      ...
      public function getPedidosEstado($estatus) {
       ...
       // die;
        ...
        $select = $this->select()
             ->from(array('e'  => 'hk_pedidos'),array('id','id_cliente','id_user' ,'total','costo','fecha'=>'DATE_FORMAT(fecha, "%d/%m/%Y")','status','descripcion_pedido'=>'descripcion')  )
             ->join(array('s' => 'hk_estatus_pedido'),'e.status = s.id', array('descripcion', 'clase', 'id_estatus_pedido'=>'id'))
            ->join(array('c' => 'hk_clientes'),'e.id_cliente = c.id', array('nombre', 'tlf', 'direccion'))
...
;                   
    ...
    /*Reportes*/
     ...
     public function reporteDia() {
        $select = $this->select()
            ->from(array('e'  => 'hk_pedidos'),array('fecha'=>'DATE_FORMAT(e.fecha, "%d/%m/%Y")','e.status', 'e.costo', 'e.total', 'total_ventas'=>'sum(e.total)', 'total_costo'=>'sum(e.costo)', 'cantidad_ventas'=>'count(e.id)', 'e.id')  )
            ->group('e.fecha');
